...
var a1list = [
    'H1.The appearance and contents of a <em><strong>Formal instrument</em></strong> includes ....... The purpose and role is to ..........',
    '1Z.The appearance and contents of <em><strong>AS4000-1997 - General conditions of contract</em></strong> includes ....... The purpose and role is to ..........',
    '43.The appearance and contents of <em><strong>AS4000-1997 - Annexure Part A</em></strong> includes ....... The purpose and role is to ..........',
    'B4.The appearance and contents of <em><strong>AS4000-1997 - Annexure Part B</em></strong> includes ....... The purpose and role is to ..........',
    '15.The appearance and contents of the <em><strong>Scope of Works</em></strong> includes ....... The purpose and role is to ..........',
    '6P.The appearance and contents of a <em><strong>Program</em></strong> includes ....... The purpose and role is to ..........',
    'KL.The appearance and contents of a <em><strong>Bill of quantities</em></strong> includes ....... The purpose and role is to ..........',
    'WK.The appearance and contents of <em><strong>Technical specifications</em></strong> includes ....... The purpose and role is to ..........',
    'G2.The appearance and contents of <em><strong>Construction drawings</em></strong> includes ....... The purpose and role is to ..........'
];
...
function generateAnswers() {
    var idtext = document.getElementById("idform").elements[1].value; //Get ID from input form
    var idcount=idtext.length;  //Start coutning at the end of the ID string
    //Generate Question and Answer 1 from IDKey
    var qdiv = document.getElementById('q1list');
    var adiv = document.getElementById('a1list');
    
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li>" + getQFromList(questid,q1list);
    adiv.innerHTML += "<ol><li>" + getAFromList(questid,a1list);
    idcount -= 2;
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value='2'>" + getQFromList(questid,q1list);
    adiv.innerHTML += "<ol><li value='2'>" + getAFromList(questid,a1list);
    idcount -= 2;
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value='3'>" + getQFromList(questid,q1list);
    adiv.innerHTML += "<ol><li value='3'>" + getAFromList(questid,a1list);

    //Generate Question and Answer 2 from IDKey. It needs to referencethe possible documents in q1 and the combination of task 2
    idcount -= 2;
    var qdiv = document.getElementById('q2list');
    var adiv = document.getElementById('a2list');
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li>" + getQFromList(questid,q2list);
    adiv.innerHTML += "<ol><li>" + getAFromList(questid,a2list);
    idcount -= 2;
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value='2'>" + getQFromList(questid,q2list);
    adiv.innerHTML += "<ol><li value='2'>" + getAFromList(questid,a2list);
    idcount -= 2;
    var qdiv = document.getElementById('q2list');
    var adiv = document.getElementById('a2list');
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value ='3'>" + getQFromList(questid,q2list);
    adiv.innerHTML += "<ol><li value ='3'>" + getAFromList(questid,a2list);
    idcount -= 2;
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value='4'>" + getQFromList(questid,q2list);
    adiv.innerHTML += "<ol><li value='4'>" + getAFromList(questid,a2list);
    idcount -= 2;
    questid = idtext.substring(idcount-2,idcount);
    qdiv.innerHTML += "<ol><li value='5'>" + getQFromList(questid,q2list);
    adiv.innerHTML += "<ol><li value='5'>" + getAFromList(questid,a2list);
}

function getQFromList (qid, qlist) {
    for (var lcount=0, tot=qlist.length; lcount < tot; lcount ++) {
        var qidfromlist = qlist[lcount].substring(0,2);
        if (qidfromlist == qid) {
            return(qlist[lcount].substring(3));
        }
    }
}

function getAFromList (qid, alist) {
    for (var lcount=0, tot=alist.length; lcount < tot; lcount ++) {
        var afromlist = alist[lcount].substring(0,2);
        if (afromlist == qid) {
            return(alist[lcount].substring(3));
        }
    } 
}
...
function generateQuestions () {
    ...
    q9div.innerHTML += '<p>Investigate the term <strong>' + injectq8list.getItem() + '</strong> and then describe <strong><em>in your own words</em></strong>:';
